...
Sensitivity_plot=function(df, fname_head){
  ...
  model=nlme(FCL ~ a*(1-exp(-b*Richness))-c*Chain^2+d*Chain-1, data=df_fill, 
...
g2= ggplot(df, aes(x = Richness, y = FCL)) +
  ...
  )+stat_function(fun = function(x) fixed_coefs[1]*(1-exp(-fixed_coefs[2]*x))-fixed_coefs[3]*mean(df$Chain, na.rm=TRUE)^2+fixed_coefs[4]*mean(df$Chain, na.rm=TRUE)-1, color='green', linewidth=2)
# show random effects
...
g3= ggplot(df, aes(x = Chain, y = FCL)) +
  ...
  theme(
    ...
    legend.text = element_text(size = 14),
    legend.title = element_text(size = 14)
  )+stat_function(fun = function(x) fixed_coefs[1]*(1-exp(-fixed_coefs[2]*mean(df$Richness, na.rm=TRUE)))-fixed_coefs[3]*x^2+fixed_coefs[4]*x-1, color='green', linewidth=2)
...
# run codes
Sensitivity_plot(read.csv('./Species32/basal4/AlternativeData/FCLmean_summary.csv'), 'FCLmean')
Sensitivity_plot(read.csv('./Species32/basal4/AlternativeData/FCLlong_summary.csv'), 'FCLlong')
Sensitivity_plot(read.csv('./Species32/basal4/AlternativeData/FCLshort_summary.csv'), 'FCLshort')
```
...
df_fill$FW=rep(1:30, each=1620) 
 model_short=nlme(FCL ~ a*(1-exp(-b*Richness))-c*Chain^2+d*Chain-1, data=df_fill, 
               start=c(a=max(df_fill$FCL)/2, b=1, c=1, d=1), 
                fixed = a+b+c+d ~ 1,
                random = a+b+c+d  ~ 1 | FW,
               control=c(maxIter=50, tolerance=1e-3, msMaxIter=100,

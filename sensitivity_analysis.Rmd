...
# Use this code to see the parms of fcl
#model_mean=nls(FCL ~ a*(1-exp(-b*Richness))-c*Chain^2+d*Chain-1, data=df, 
#              start=c(a=max(df$FCL)/2, b=4, c=1, d=1), 
#              algorithm='port', lower=c(a=0, b=0, c=0, d=0), 
#              upper=c(a=max(df$FCL)+1, b=10, c=10, d=10))
#summary(model_mean)

Sensitivity_plot(df, fname_head){
  ...
  g3=ggplot(df, aes(x=Richness, y=FCL))+ylab('FCL')+geom_bin2d()+scale_fill_continuous(low='gray90', high='black')+
  theme_bw()+theme(
...
panel.grid = element_blank(),
 legend.text=element_text(size=14),
  ...
  g5=ggplot(data, aes(Ecosystem, Disturbance, fill= Richness), fig(8,8)) + 
  ...
  g6=ggplot(data, aes(Ecosystem, Disturbance, fill= FCL), fig(8,8)) + 
    xlab('Ecosystem size')+ylab("Disturbance")+
    geom_tile(color = "black") +scale_y_continuous(breaks=seq(0.2,0.8,by=0.2))+
...
```{r Model selection with the sensitivity analysis}
df=read.csv('./Sensitivity_analysis/Summary_FCLmean.csv')
df[is.na(df)]=0 # na is the fraction of motifs when richness<3

...
Note that 13 motives are defined but this funciton returns 16 motives because 
the function can include sugraphs that include isolated nodes.
...
FCL=function(A, def=0){
  ...
  # def: defnition of FCL. We chosoe one definition from below
  # def =-1: shortest-path
  # def =0 (default)  mean trphic levels of prey +1. Only acyclic graph
  # def = 1: longest-path. Only acyclic graph
  
  N = nrow(A) # number of species in the food web
  TL=rep(0, N)
  B=c()
  # Check the number of basal species
  for (i in 1:N){
    if (sum(A[, i])==0){
...
library(ggplot2)
library(tidyverse)
data=read.csv("Cohen2010_summary.csv")
...
head(df)
# Below analyze df. We may need to convert the numbers of the motives into their fractions
...
# s3 and d5-d8 are missing in all data
#print(apply(df[, 4:ncol(df)], 2, sum))
df = subset(df, select = -c(s3, d5, d6, d7,d8) )
df[, 4:ncol(df)] = scale(df[,4:ncol(df)])
# check correlation
library(ggcorrplot)
...
Although we cannot analyze cyclic food webs, analyzing FCL_long would be more interesting than FCL_short because the variation in long is larger than short one. In addition, we can see some correlations between FCLs and fractions of s-motives
...
summary(lm(formula = FCL_long ~ richness + s1 + s2 + s4, data = df)) # we use s5 as intercept
``` 
As the variations in FCL-short is small, the regression is not interesting as expected. In contrast, we can see the effects of the single-path motives on FCL_long. Let's us see next the comparisons between the prediction and acutusl FCL-long.
Note: linear regression would not be a good choice as FCL is discrete.
In addition, we should standarize the exp. variables  so that we can comapre the effects. Poisson? mean of FZCL_long is 3.7 and variance is 2.3, but this family cannot generate FCL>3

```{r diag}
library(ggfortify)
model=glm(FCL_long~richness+s1+s2+s5, data=df, family = 'gaussian')
summary(model)
autoplot(model, label.size = 3)

plot(data$FCL_long[-(which(is.na(data$FCL_long)))],predict(model), 
     xlab='FCL_long', ylab='Prediction',

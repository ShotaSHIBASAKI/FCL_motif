...
mean_chain=mean(df_mean$chain)
print(mean_chain)
mean_richness=mean(df_mean$richness)
print(mean_richness)
g1=ggplot(df_mean, aes(x=richness, y=FCL, size=3, alpha=0.000005))+geom_point(color='black')+ylab('FCL mean')+xlab("Richness")+theme(
axis.text.x = element_text(size=14),
  axis.title.x = element_text(size=20),
   axis.text.y = element_text(size=14),
  axis.title.y = element_text(size=20),
  legend.position="none"
  )+stat_function(fun = function(x) 3.86*(1-exp(-0.2*x))-0.41*mean_chain^2+1.73*mean_chain-1, color='red')
g2=ggplot(df_mean, aes(x=chain, y=FCL, size=3, alpha=0.000005))+geom_point(color='black')+ylab('FCL mean')+xlab("Chain")+theme(
axis.text.x = element_text(size=14),
  axis.title.x = element_text(size=20),
   axis.text.y = element_text(size=14),
  axis.title.y = element_text(size=20),
  legend.position="none"
  )+stat_function(fun = function(x) 3.86*(1-exp(-0.2*mean_richness))-0.41*x^2+1.73*x-1, color='red')
df_mean$predict=3.86*(1-exp(-0.2*df_mean$richness))-0.41*df_mean$chain^2+1.73*df_mean$chain-1
g3=ggplot(df_mean, aes(x=FCL, y=predict, size=3, alpha=0.000005))+geom_point(color='black')+ylim(c(-1, 6))+xlab('FCL mean')+ylab("Model prediction")+theme(
axis.text.x = element_text(size=14),
  axis.title.x = element_text(size=20),
   axis.text.y = element_text(size=14),
  axis.title.y = element_text(size=20),
...
ggsave('Modeling_FCLmean.png', width=12, height=8)
```
...
AIC_cal2= function(df){
  ...
  model4=glm(formula = FCL + 1 ~ chain, family = poisson(link = "log"), 
    data = df_long)# linear func of chain
  model5=nls(FCL ~ a*(1-exp(-b*chain))-1, data=df, start=c(a=max(df$FCL)/2, b=b_start),  
             algorithm='port', lower=c(a=0, b=0), upper=c(a=max(df$FCL)+1, b=10)) # saturating func of chain
  model6=nls(FCL ~ -a*chain^2+b*chain-1, data=df, start=c(a=1, b=1), 
             algorithm='port', lower=c(a=0, b=0), upper=c(a=10, b=10)) # quadratic func of chain

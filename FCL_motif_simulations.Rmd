...
Main= function(TL_def, num_FW=30){
  ...
  for (n in 1:num_FW){
    ...
    presence_array=data[, 4:ncol(data)]
    ...
    D=rbind(D, cbind(FW=n, data[, 1:3], ans))
  }
  return(D)
}
data_long=Main(1)
...
```{r FCL analysis}
library(ggcorrplot)
Plot_corr=function(data, FCL_name, fname){
  df=subset(data, select = c(FCL, richness, chain, omnivory, apparent, exploitative, resource, disturbance) )
  colnames(df) = c(FCL_name, 'Richness', 
                 'Chain', 'Omnivory', 'Apparent comp', 'Exploitative comp', 
                 'Resource', 'Disturbance')
  corr=cor(df, use = "pairwise.complete.obs", method='pearson') # pearson and spearman show similar results
  print(corr)
...
Plot_corr(data_short, 'FCL short', 'Simulated_FCL_short_correlation.pdf')
...
```{r}
print('FCL long')
ggplot(data_long, aes(x=richness)) + geom_bar()+xlab('Richness')+ylab('Count')
ggplot(data_long, aes(x=FCL)) + geom_bar()+xlab('FCL long')+ylab('Count')

print('FCL mean')
ggplot(data_mean, aes(x=richness)) + geom_bar()+xlab('Richness')+ylab('Count')
ggplot(data_mean, aes(x=FCL)) + geom_histogram()+xlab('FCL mean')+ylab('Count')

print('FCL short')
ggplot(data_short, aes(x=richness)) + geom_bar()+xlab('Richness')+ylab('Count')
ggplot(data_short, aes(x=FCL)) + geom_bar()+xlab('FCL short')+ylab('Count')
